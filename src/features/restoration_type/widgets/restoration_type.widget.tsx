...
import { importExcel } from "@features/restoration_type/api/restoration_type.api";
import { reloadTable } from "@core/table/table-reload";

function RestorationTypeWidget() {
  ...
  return (
    <>
      <SectionCard extra={
          ...
          <Stack direction="row" spacing={1}>
            <IfPermission permissions={["product.create"]}>
              <Button variant="outlined" startIcon={<AddIcon />} onClick={() => {
                openFormDialog("restoration_type");
              }} >Thêm kiểu phục hình</Button>
            </IfPermission>
            <IfPermission permissions={["product.create"]}>
              <Button
                variant="outlined"
        ...
        <AutoTable name="restoration_types" />
      </SectionCard>
    </>
  );
}
...
registerSlot({
  id: "restoration_type",
  name: "restoration_type:left",
  render: () => <RestorationTypeWidget />,
});
